...
public class MainAttraction extends AppCompatActivity {
    ...
    private int count = 6; // кількість елементів на сторінці
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.listView = findViewById(R.id.listViewAttractions);
    ...
    public void add_btn_clicked(View view) {
        String name = ((EditText) findViewById(R.id.editTextNameAttraction)).getText().toString();
        String type = ((EditText) findViewById(R.id.editTextType)).getText().toString();
        boolean isOpened = ((Switch)findViewById(R.id.switchIsOpened)).isChecked();

        DatabaseConnector databaseConnector = new DatabaseConnector(this);

        if (currentItemID > 0) {
            databaseConnector.editTableRow(currentItemID, name, type, isOpened);
            currentItemID = 0;
        } else {
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenu.ContextMenuInfo menuInfo) {
        if (view.getId() == R.id.listViewAttractions) {
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item){
        ...
        switch (item.getItemId()) {
            ...
            case MENU_CONTEXT_EDIT_ID:{
                ...
                ((EditText)findViewById(R.id.editTextNameAttraction)).setText(str.split(", ")[1]);
                ((EditText)findViewById(R.id.editTextType)).setText(str.split(", ")[2]);
                return true;
            }
            default:
